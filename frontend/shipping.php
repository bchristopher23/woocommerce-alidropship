...
class VI_WOOCOMMERCE_ALIDROPSHIP_Frontend_Shipping {
	...
	public function __construct() {
		...
		self::$language = '';
		if ( $this->settings->get_params( 'enable' ) && $this->settings->get_params( 'ali_shipping' ) ) {
			...
			if ( $this->settings->get_params( 'ali_shipping_product_enable' ) ) {
				...
				if ( $this->settings->get_params( 'ali_shipping_product_position' ) === 'before_cart' ) {
	...
	private function single_product_shipping_html( $product, $quantity, $variation_id = '' ) {
				...
				if ( ! $variation_id ) {
					if ( count( $ship_froms ) > 1 ) {
                        ...
                        <div class="vi-wad-single-product-shipping-wrap vi-wad-single-product-shipping-need-select-variation">
                            ...
                            <div class="vi-wad-single-product-shipping-error"><?php echo str_replace( '{country}', $country_name, $this->settings->get_params( 'ali_shipping_select_variation_message', self::$language ) ) ?></div>
                        </div>
						<?php
						return;
		...
		if ( $ali_id ) {
			...
			$ali_shipping_display = $this->settings->get_params( 'ali_shipping_product_display' );
			ob_start();
			if ( $shipping_info['shipping_cost'] === '' && $this->settings->get_params( 'ali_shipping_not_available_remove' ) ) {
				$class .= ' vi-wad-single-product-shipping-not-available';
				?>
                <div class="vi-wad-single-product-shipping-error">
					<?php
					echo str_replace( '{country}', $country_name, $this->settings->get_params( 'ali_shipping_product_not_available_message', self::$language ) );
					?>
                </div>
				<?php
			} else {
                ...
                <div class="vi-wad-single-product-shipping-label">
					<?php
					echo str_replace( '{country}', $country_name, $this->settings->get_params( 'ali_shipping_product_text', self::$language ) );
					?>
                </div>
				<?php
	...
	public function woocommerce_package_rates( $methods, $package ) {
		if ( ! empty( $package['contents'] ) ) {
			$ali_shipping_type = $this->settings->get_params( 'ali_shipping_type' );
			if ( $ali_shipping_type !== 'none' ) {
				$ali_total_shipping = 0;
				$ali_shipping       = false;
				$items_in_package   = array();
				$remove_cart_item   = $this->settings->get_params( 'ali_shipping_not_available_remove' );
				$not_available_cost = apply_filters( 'vi_wad_frontend_item_not_available_cost', apply_filters( 'wmc_change_3rd_plugin_price', $this->settings->get_params( 'ali_shipping_not_available_cost' ) ) );
				foreach ( $package['contents'] as $cart_item_key => $cart_item ) {
					if ( isset( $cart_item['vi_wad_item_shipping'] ) ) {
						...
						if ( $shipping_info['shipping_cost'] !== '' ) {
							...
							$shipping_info['shipping_cost'] = $this->settings->process_exchange_price( $shipping_info['shipping_cost'] );
				...
				if ( $ali_shipping ) {
					if ( $ali_total_shipping ) {
						...
						$id                 = 'flat_rate';
						$label              = $this->settings->get_params( 'ali_shipping_label', self::$language );
						if ( ! $label ) {
							$label = esc_html__( 'Shipping', 'woocommerce-alidropship' );
						}
					} else {
						$id    = 'free_shipping';
						$label = $this->settings->get_params( 'ali_shipping_label_free', self::$language );
						if ( ! $label ) {
...
//				}
	...
	public function wp_enqueue_scripts() {
		if ( is_cart() || is_checkout() || ( is_product() && $this->settings->get_params( 'ali_shipping_product_enable' ) ) ) {
			if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) ) {
				$default_lang     = apply_filters( 'wpml_default_language', null );
				$current_language = apply_filters( 'wpml_current_language', null );
				if ( $current_language && $current_language !== $default_lang ) {
	...
	public function woocommerce_get_item_data( $item_data, $cart_item ) {
		if ( ! $this->is_minicart && $this->cart_item_key && isset( $cart_item['vi_wad_item_shipping'] ) ) {
			$shipping_info        = $cart_item['vi_wad_item_shipping'];
			$ali_shipping_display = $this->settings->get_params( 'ali_shipping_display' );
			$item_data[]          = array(
	...
	public function show_shipping_selection( $shipping_info, $ali_shipping_display, $cart_item_key, $container_class = '' ) {
		if ( $shipping_info['shipping_cost'] === '' ) {
			...
			if ( count( $shipping_info['freight'] ) ) {
				$option_html                = '';
				$popup_item                 = '';
				$ali_shipping_show_tracking = $this->settings->get_params( 'ali_shipping_show_tracking' );
				$option_text                = $this->settings->get_params( 'ali_shipping_option_text', self::$language );
				if ( ! $option_text ) {
					$option_text = $this->settings->get_default( 'ali_shipping_option_text' );
				}
				$mask_company = $this->settings->get_params( 'ali_shipping_company_mask' );
				if ( $mask_company ) {
				...
				foreach ( $shipping_info['freight'] as $freight_k => $freight_v ) {
					...
					if ( $freight_v['shipping_cost'] == 0 ) {
						...
						$freight_v['shipping_cost'] = $this->settings->process_exchange_price( $freight_v['shipping_cost'] );
	...
	private function get_default_shipping_message() {
		$not_available_message = $this->settings->get_params( 'ali_shipping_not_available_message', self::$language );
		$remove_cart_item      = $this->settings->get_params( 'ali_shipping_not_available_remove' );
		if ( $remove_cart_item ) {
			...
			$default_shipping_message = str_replace( array(
				...
				$not_available_cost,
				VI_WOOCOMMERCE_ALIDROPSHIP_Admin_Import_List::process_delivery_time( $this->settings->get_params( 'ali_shipping_not_available_time_min' ) . '-' . $this->settings->get_params( 'ali_shipping_not_available_time_max' ) )
			), $not_available_message );
		}
...
//		$country  = isset( $customer['shipping_country'] ) ? $customer['shipping_country'] : '';
	...
	public function woocommerce_checkout_create_order_line_item( $item, $cart_item_key, $values, $order ) {
		if ( isset( $values['vi_wad_item_shipping'] ) ) {
			$cost_added = false;
			switch ( $this->settings->get_params( 'ali_shipping_type' ) ) {
				case 'new':
			...
			if ( ! $shipping_info['shipping_cost'] ) {
				if ( ! $shipping_info['company'] ) {
					if ( ! $this->settings->get_params( 'ali_shipping_not_available_remove' ) ) {
						$shipping_info['shipping_cost'] = apply_filters( 'vi_wad_frontend_item_not_available_cost', apply_filters( 'wmc_change_3rd_plugin_price', $this->settings->get_params( 'ali_shipping_not_available_cost' ) ) );
						$shipping_info['delivery_time'] = $this->settings->get_params( 'ali_shipping_not_available_time_min' ) . '-' . $this->settings->get_params( 'ali_shipping_not_available_time_max' );
					}
				}
			} else {
				$shipping_info['shipping_cost'] = $this->settings->process_exchange_price( $shipping_info['shipping_cost'] );

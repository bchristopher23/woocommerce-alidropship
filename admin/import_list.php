			...
			if ( $override_woo_id && $migration_link_only ) {
				...
				if ( $woo_product->is_type( 'variable' ) ) {
					...
					foreach ( $replace_items as $variation_id => $replace_items_v ) {
						...
						if ( $variations_key !== false ) {
							...
							if ( $product_obj && in_array( $variation_id, $children ) ) {
								if ( ! empty( $product_variation['skuAttr'] ) ) {
					...
					if ( $variations_key !== false ) {
						$product_variation = $variations[ $variations_key ];
						if ( ! empty( $product_variation['skuAttr'] ) ) {
							update_post_meta( $woo_product_id, '_vi_wad_aliexpress_variation_attr', $product_variation['skuAttr'] );
							if ( ! empty( $product_variation['ship_from'] ) ) {
								update_post_meta( $woo_product_id, '_vi_wad_aliexpress_variation_ship_from', $product_variation['ship_from'] );
							}
						}
					}
				}
				...
				if ( $override_keep_product ) {
					...
					if ( $is_simple ) {
						...
						} elseif ( get_post_meta( $woo_product_id, '_vi_wad_aliexpress_variation_attr', true ) ) {
						...
						} elseif ( get_post_meta( $woo_product_id, '_vi_wad_aliexpress_variation_ship_from', true ) ) {
						...
						if ( ! empty( $variations[0]['stock'] ) && $manage_stock === 'yes' ) {
							$woo_product->set_stock_quantity( $variations[0]['stock'] );
						}
						$woo_product->save();
						wp_set_object_terms( $woo_product_id, 'simple', 'product_type' );
						if ( $dispatch ) {
...
//											}
											...
											if ( ! empty( $product_variation['ship_from'] ) ) {
												update_post_meta( $variation_id, '_vi_wad_aliexpress_variation_ship_from', $product_variation['ship_from'] );
											} elseif ( get_post_meta( $variation_id, '_vi_wad_aliexpress_variation_ship_from', true ) ) {
												delete_post_meta( $variation_id, '_vi_wad_aliexpress_variation_ship_from' );
											}
										}
									} else {
										wp_delete_post( $variation_id, true );
...
//					wp_set_post_terms( $product_id, $tags, 'product_tag', true );
				...
				if ( ! empty( $variations[0]['ship_from'] ) ) {
					update_post_meta( $product_id, '_vi_wad_aliexpress_variation_ship_from', $variations[0]['ship_from'] );
				}
...
//				$attributes = get_post_meta( $product_id, '_vi_wad_attributes', true );
	...
	public function background_process() {
		self::$process              = new VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_IMPORT_PRODUCT();
		self::$process_image        = new VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_DOWNLOAD_IMAGES();
		self::$download_description = new VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_DOWNLOAD_DESCRIPTION();

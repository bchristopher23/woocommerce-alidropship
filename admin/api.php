	...
	public function add_to_import_list( $data, $shipping_info ) {
		...
		if ( ! $post_id ) {
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$result['message'] = esc_html__( 'Can not create post', 'woocommerce-alidropship' );
				wp_send_json( $result );
			}
						...
						if ( ! $post_id ) {
							...
							if ( $get_data['status'] === 'success' ) {
								...
								if ( is_wp_error( $post_id ) ) {
									...
									$detail['message'] = esc_html__( 'Can not create post', 'woocommerce-alidropship' );
								} else {
					...
					foreach ( $matchOrders as $matchOrder ) {
						...
						foreach ( $order_items as $item_id => $item ) {
							...
							if ( in_array( $ali_product_id, $matchProductIds ) ) {
								...
								/*Set aliexpress order id if payment has been made*/

						...
						if ( $product->is_type( 'variable' ) ) {
							...
							if ( ! $is_simple ) {
								...
								if ( count( $mapped_variations ) === count( $children ) ) {
									foreach ( $mapped_variations as $variation_id => $variation_key ) {
										update_post_meta( $variation_id, '_vi_wad_aliexpress_variation_attr', $variations[ $variation_key ]['skuAttr'] );
										if ( ! empty( $variations[ $variation_key ]['ship_from'] ) ) {
											update_post_meta( $variation_id, '_vi_wad_aliexpress_variation_ship_from', $variations[ $variation_key ]['ship_from'] );
							...
							if ( ! $is_simple ) {
								...
								if ( $sku_props ) {
									...
									if ( $search !== false ) {
										$import = true;
										update_post_meta( $woo_id, '_vi_wad_aliexpress_variation_attr', $variations[ $search ]['skuAttr'] );
										if ( ! empty( $variations[ $search ]['ship_from'] ) ) {
											update_post_meta( $woo_id, '_vi_wad_aliexpress_variation_ship_from', $variations[ $search ]['ship_from'] );
										} elseif ( get_post_meta( $woo_id, '_vi_wad_aliexpress_variation_ship_from', true ) ) {
											delete_post_meta( $woo_id, '_vi_wad_aliexpress_variation_ship_from' );
										}
									}
								}
							} else {
...
//		VI_WOOCOMMERCE_ALIDROPSHIP_Admin_Log::log( $content, 'migrate_products.txt' );

...
class VI_WOOCOMMERCE_ALIDROPSHIP_Admin_Vendor {
	...
	public function __construct() {
		...
		if ( self::$settings->get_params( 'enable' ) && self::enable_vendor_integration() ) {
			...
			add_filter( 'vi_wad_import_list_button_view_edit_html', array(
				$this,
				'vi_wad_import_list_button_view_edit_html'
			), 10, 2 );
			add_action( 'vi_wad_import_list_product_information', array(
				$this,
				'add_vendor_info'
			) );
			add_action( 'vi_wad_imported_list_product_information', array(
				$this,
				'add_vendor_info'
			) );
			add_action( 'vi_wad_import_list_product_message', array(
				$this,
				'add_vendor_settings_notice'
			), 10, 3 );
			add_action( 'vi_wad_import_list_product_message', array(
				$this,
				'dokan_lite_not_support_variable_notice'
			), 10, 3 );
			...
			add_filter( 'vi_wad_product_sync_email_headers', array(
				$this,
				'maybe_send_bcc_of_email_to_vendor'
			), 10, 4 );

			add_action( 'dokan_enqueue_scripts', array(
				$this,
				'vendor_dashboard_general'
			) );
			...
			add_action( 'dokan_product_after_variable_attributes', array(
				$this,
	...
	/**
	 * Do not allow to import variable products if Dokan pro is not active because Dokan lite only support simple products
	 *
	 * @param $product_data
	 */

...
if ( ! class_exists( 'VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_IMPORT_PRODUCT' ) ) {
	class VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_IMPORT_PRODUCT extends WP_Background_Process {
		/**
		 * @var string
		 */
				...
				if ( is_array( $attributes ) && count( $attributes ) ) {
					...
					foreach ( $attributes as $key => $attr ) {
						$attr_data[ strtolower( $attr['slug'] ) ] = array(
							'name'         => VI_WOOCOMMERCE_ALIDROPSHIP_DATA::get_attribute_name_by_slug( $attr['slug'] ),
					...
					$product_data = array( // Set up the basic post data to insert for our product
						...
						'post_type'    => 'product',
						'meta_input'   => array(
							'_sku'                => VI_WOOCOMMERCE_ALIDROPSHIP_DATA::sku_exists( $sku ) ? '' : $sku,
					...
					$product_id = wp_insert_post( $product_data, true ); // Insert the post returning the new post id
					...
					if ( ! is_wp_error( $product_id ) ) {
						...
						if ( $image ) {
							...
							VI_WOOCOMMERCE_ALIDROPSHIP_Admin_Import_List::$process_image->push_to_queue( $image_data );
						}
					...
					if ( ! is_wp_error( $product_id ) ) {
						...
						if ( $dispatch ) {
							VI_WOOCOMMERCE_ALIDROPSHIP_Admin_Import_List::$process_image->save()->dispatch();
						}
						/*Set product tag*/
						if ( is_array( $tags ) && count( $tags ) ) {
			...
			if ( $product ) {
				if ( is_array( $item['variations'] ) && count( $item['variations'] ) ) {
					...
					foreach ( $item['variations'] as $product_variation ) {
						...
						$fields = array(
							'sku'            => VI_WOOCOMMERCE_ALIDROPSHIP_DATA::sku_exists( $product_variation['sku'] ) ? '' : $product_variation['sku'],
						...
						if ( ! empty( $product_variation['ship_from'] ) ) {
							update_post_meta( $variation->get_id(), '_vi_wad_aliexpress_variation_ship_from', $product_variation['ship_from'] );
						}
		 ...
		 * Delete all batches.
		 *
		 * @return VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_IMPORT_PRODUCT
		 */

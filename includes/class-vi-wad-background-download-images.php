...
class VI_WOOCOMMERCE_ALIDROPSHIP_BACKGROUND_DOWNLOAD_IMAGES extends WP_Background_Process {
	...
	protected function task( $item ) {
		...
		try {
			...
			if ( $post && isset( $post->post_type ) && $post->post_type === 'product' && $src ) {
				...
				if ( $thumb_id && ! is_wp_error( $thumb_id ) ) {
					...
					error_log( 'ALD - AliExpress Dropshipping and Fulfillment for WooCommerce error: ' . $thumb_id->get_error_code() . ' - ' . $thumb_id->get_error_message() );
				}
			}
			...
			VI_WOOCOMMERCE_ALIDROPSHIP_Error_Images_Table::insert( $product_id, implode( ',', $product_ids ), $src, intval( $set_gallery ) );
			error_log( 'ALD - AliExpress Dropshipping and Fulfillment for WooCommerce error: ' . $e->getMessage() );
